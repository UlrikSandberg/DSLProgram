...
const VIPsOverviewPage = () => {
        ...
        const fetchData = async () => {
            ...
            setLoading(true);
            var result = await httpGet<VIP[]>("/VIP")
        ...
        const deleteEntity = async (id: String) => {
	                ...
	                var result = await httpDelete<boolean>(`/VIP?id=${id}`)
	                
	                if(result.isSuccess) {
                    	VIPResult.splice(VIPResult.indexOf(VIPResult.filter(e => e.id === id)[0]),1)
                    	setVIPResult([...VIPResult]);
	                } 
                    ...
                    <Grid container justify="center">
                        <Grid item xs={12} sm={12} md={12} lg={12} xl={12} style={{padding: "20px"}}>
                            <div style={{width: "100%", display: "flex", justifyContent: "center", flexDirection: "column"}}>
                                <Typography variant="h4">VIP overview</Typography>
                                <Card>
                                    <TableContainer style={{margin: "20px 0", padding: "20px", overflow: "true"}}>
                                        <Table aria-label="simple table">
                                            <TableHead>
                                            <TableRow>
                                            	<TableCell >VIPMember</TableCell>
                                            	<TableCell></TableCell>
                                            	<TableCell></TableCell>
                                            </TableRow>
                                            </TableHead>
                                            ...
                                            {VIPResult.map((row) => (
                                                <TableRow key={row.id}>
                                                    ...
                                                    <TableCell>
                                                    	<Button color="primary" variant="outlined" onClick={() => history.push(`/management/VIP_update/${row.id}`)}>Edit</Button>
                                                    </TableCell>
                                            		<TableCell>
