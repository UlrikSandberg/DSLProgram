...
import { UpdateRegularSeatScheduleRequestModel } from "../../../api/requestModels/UpdateRegularSeatScheduleRequestModel"
import { RegularSeatSchedule } from "../../../api/models/schedules/RegularSeatSchedule";

const UpdateRegularSeatSchedulePage = () => {
...
const params = useParams() as { id: string }
	...
	const [loadResult, setLoadResult] = useState<RegularSeatSchedule>();
    ...
    const load = async () => {
	            ...
	            const result = await httpGet<RegularSeatSchedule>(`/RegularSeatSchedule/${params.id}`)
	            if(result.isSuccess) {
	                setLoadResult(result.data)
setname(result.data.name)
setstartTimeEpoch(result.data.startTimeEpoch)
setendTimeEpoch(result.data.endTimeEpoch)
setDescription(result.data.Description)
	...
	const submit = async () => {
        ...
        const result = await httpPut<UpdateRegularSeatScheduleRequestModel>("/RegularSeatSchedule", {
        	id: params.id,
            name: name, startTimeEpoch: startTimeEpoch, endTimeEpoch: endTimeEpoch, Description: Description
        } as UpdateRegularSeatScheduleRequestModel);
                    ...
                    <Grid container style={{width: "100%", minHeight: "100vh"}} justify="center" alignItems="center">
                        <Grid item xs={10} sm={8} md={6} lg={4} xl={4}>
                            <Card style={{width: "100%", padding: "20px", display: "flex", justifyContent: "center", flexDirection: "column", textAlign: "center"}}> 
                                <Typography style={{paddingBottom: "10px"}} variant="h5">Update RegularSeatSchedule</Typography>
                                {success
	                            ? <Alert style={{margin: "10px 0"}} severity="success">
	                                <AlertTitle>Success</AlertTitle>
	                                RegularSeatSchedule was updated successfully
	                            </Alert>
	                            : null}
	                            {error || loadError
...
export default UpdateRegularSeatSchedulePage;
